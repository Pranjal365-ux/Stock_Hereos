...
import { StockStatus, EnrichedStock } from '../hooks/useStockData';
...
export default function VirtualTrading({ onBack, virtualCash, portfolio, onBuyStock, onSellStock, stockData }: VirtualTradingProps) {
  const { stocks, chartData, status, lastRefreshed, refresh, getStockById } = stockData;
  ...
  const handleBuy = () => {
    const result = onBuyStock(selectedStockId, quantity, livePrice);
  ...
  const handleSell = () => {
    const result = onSellStock(selectedStockId, quantity, livePrice);
    setMessage({ text: result.message, type: result.success ? 'success' : 'error' });
    if (result.success) setQuantity(1);
    setTimeout(() => setMessage(null), 3000);
  };
    ...
    <div className="screen-bg min-h-screen p-6">
      ...
      <div className="mb-5 animate-slide-up relative z-10">
        ...
        <div className="glass-card px-4 py-2 inline-flex items-center gap-2 mt-3">
          ...
          <span className="text-white text-base font-semibold">₹{virtualCash.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>
        </div>
      </div>
      ...
      <div className="glass-card p-5 mb-4 animate-slide-up delay-200 relative z-10">
        <div className="flex items-end gap-2 mb-1">
          <span className="text-white text-3xl font-semibold">₹{livePrice.toFixed(2)}</span>
        ...
        {holding && holding.quantity > 0 && (
          <div className="mt-3 pt-3" style={{ borderTop: '1px solid rgba(133,79,108,0.2)' }}>
            ...
            <p style={{ color: (livePrice - holding.avgPrice) >= 0 ? '#4ade80' : '#f87171', fontSize: '0.85rem' }}>
              Unrealised P&L: {(livePrice - holding.avgPrice) >= 0 ? '+' : ''}₹{((livePrice - holding.avgPrice) * holding.quantity).toFixed(0)}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="glass-card p-5 mb-4 animate-slide-up delay-300 relative z-10">
        <div className="flex items-center justify-between mb-4">
          ...
          {rawChart.length >= 2 && <span style={{ color: 'rgba(223,182,178,0.4)', fontSize: '0.7rem' }}>{rawChart.length} pts · 5min</span>}
        </div>
      ...
      <div className="glass-card p-5 mb-5 animate-slide-up delay-400 relative z-10">
        ...
        <div className="rounded-xl p-3 text-center" style={{ background: 'rgba(82,43,91,0.3)', border: '1px solid rgba(133,79,108,0.2)' }}>
          ...
          <p className="text-white text-xl font-semibold">₹{totalCost.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</p>
        </div>
      </div>

      {/* Buy/Sell */}
      <div className="grid grid-cols-2 gap-3 relative z-10">
        <button onClick={handleBuy} disabled={totalCost > virtualCash}
          className="py-4 rounded-2xl font-semibold text-white transition-all"
          style={{ background: 'linear-gradient(135deg, rgba(34,197,94,0.8), rgba(21,128,61,0.9))', border: '1px solid rgba(34,197,94,0.4)', boxShadow: '0 4px 20px rgba(34,197,94,0.25)', opacity: totalCost > virtualCash ? 0.4 : 1 }}>
          Buy
        </button>
      ...
      {totalCost > virtualCash && <p className="text-center mt-3 text-sm" style={{ color: 'rgba(248,113,113,0.7)' }}>Insufficient funds to buy</p>}
